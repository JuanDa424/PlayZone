...
class _MainAdminAppPageState extends State<MainAdminAppPage> {
  ...
  Widget _buildView() {
    switch (_selectedIndex) {
      case 0: return _DashboardView(service: _service, currency: _currency);
      case 1: return _UsuariosView(service: _service, search: _searchText,
                    onRefresh: () => setState(() {}));
      case 2: return _CanchasView(service: _service, search: _searchText,
                    onRefresh: () => setState(() {}));
      case 3: return _ReservasView(service: _service, search: _searchText,
                    currency: _currency, onRefresh: () => setState(() {}));
      case 4: return const _ConfigView();
      default: return _DashboardView(service: _service, currency: _currency);
    }
  }
          ...
          return Padding(
            ...
            child: InkWell(
              ...
              child: AnimatedContainer(
                ...
                child: Row(children: [
                  Icon(item.icon,
                      color: sel ? kGreenNeon : kLightGray, size: 18),
...
class _DashboardViewState extends State<_DashboardView> {
  ...
  @override
  Widget build(BuildContext context) {
    if (_loading) return const Center(child: CircularProgressIndicator(color: kGreenNeon));
    final s = _stats;
    if (s == null) return const Center(child: Text('Error cargando datos', style: TextStyle(color: kLightGray)));
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Resumen general',
                  style: TextStyle(color: kLightGray.withOpacity(0.6), fontSize: 12)),
              ...
              GridView.count(
                ...
                children: [
                  _KpiCard(icon: Icons.people_rounded, label: 'Usuarios',
                      value: s.totalUsuarios.toString(), color: kGreenNeon, index: 0),
                  _KpiCard(icon: Icons.sports_soccer_rounded, label: 'Canchas',
                      value: s.totalCanchas.toString(), color: kOrangeAccent, index: 1),
                  _KpiCard(icon: Icons.calendar_today_rounded, label: 'Reservas hoy',
                      value: s.reservasHoy.toString(), color: const Color(0xFF6B9FFF), index: 2),
                  _KpiCard(icon: Icons.check_circle_rounded, label: 'Activas',
                      value: s.reservasActivas.toString(), color: kGreenNeon, index: 3),
                  _KpiCard(icon: Icons.cancel_rounded, label: 'Canceladas',
                      value: s.reservasCanceladas.toString(), color: Colors.redAccent, index: 4),
                  _KpiCard(icon: Icons.receipt_rounded, label: 'Total reservas',
                      value: s.totalReservas.toString(), color: kOrangeAccent, index: 5),
                  _KpiCard(icon: Icons.attach_money_rounded, label: 'Ingresos totales',
                      value: widget.currency.format(s.ingresosTotales),
                      color: kGreenNeon, index: 6),
                  _KpiCard(icon: Icons.today_rounded, label: 'Ingresos hoy',
                      value: widget.currency.format(s.ingresosHoy),
                      color: const Color(0xFF6B9FFF), index: 7),
                ],
              ),
              ...
              Row(children: [
                ...
                const Spacer(),
                Text('${_recientes.length}',
                    style: const TextStyle(color: kLightGray, fontSize: 12)),
              ]),
              const SizedBox(height: 10),

              ..._recientes.asMap().entries.map((e) =>
                  _ReservaRow(reserva: e.value, currency: widget.currency, index: e.key)
...
class _UsuariosView extends StatefulWidget {
  ...
  const _UsuariosView({required this.service, required this.search, required this.onRefresh});
  @override
...
class _UsuariosViewState extends State<_UsuariosView> {
  ...
  List<_Usuario> get _filtrados => _usuarios.where((u) =>
      widget.search.isEmpty ||
      u.nombre.toLowerCase().contains(widget.search.toLowerCase()) ||
      u.correo.toLowerCase().contains(widget.search.toLowerCase())).toList();

  Color _rc(String role) {
    switch (role.toUpperCase()) {
      case 'APP_ADMIN': return kOrangeAccent;
      case 'CANCHA_ADMIN': return const Color(0xFF6B9FFF);
      default: return kGreenNeon;
    }
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            const Text('Usuarios', style: TextStyle(color: kWhite, fontSize: 18, fontWeight: FontWeight.bold)),
            const Spacer(),
            Text('${_filtrados.length}', style: const TextStyle(color: kLightGray, fontSize: 12)),
            IconButton(icon: const Icon(Icons.refresh_rounded, color: kGreenNeon, size: 20), onPressed: _cargar),
          ]),
          const SizedBox(height: 10),
          if (_loading)
            const Expanded(child: Center(child: CircularProgressIndicator(color: kGreenNeon)))
          else
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (ctx, i) {
                  ...
                  final rc = _rc(u.role);
                  final initial = u.nombre.isNotEmpty ? u.nombre[0].toUpperCase() : '?';
                  return Container(
                    ...
                    child: Row(children: [
                      Container(
                        ...
                        child: Center(child: Text(initial,
                            style: TextStyle(color: rc, fontWeight: FontWeight.bold, fontSize: 15))),
                      ),
                      const SizedBox(width: 10),
                      ...
                      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                        ...
                        const SizedBox(height: 4),
                        Icon(u.emailVerified ? Icons.verified_rounded : Icons.mail_outline_rounded,
                            color: u.emailVerified ? kGreenNeon : kLightGray, size: 14),
                      ]),
                      IconButton(
                        ...
                        onPressed: () async {
                          final ok = await showDialog<bool>(
                            ...
                            builder: (ctx) => AlertDialog(
                              ...
                              title: const Text('¿Eliminar?', style: TextStyle(color: kWhite)),
                              content: Text('${u.nombre} será eliminado.',
                          ...
                          if (ok == true) { await widget.service.eliminarUsuario(u.id); _cargar(); }
                        },
                      ),
                    ]),
...
class _CanchasView extends StatefulWidget {
  ...
  final String search;
  final VoidCallback onRefresh;
  const _CanchasView({required this.service, required this.search, required this.onRefresh});
  @override
...
class _CanchasViewState extends State<_CanchasView> {
  ...
  Future<void> _cargar() async {
    try {
      ...
      setState(() { _canchas = data; _loading = false; });
    } catch (_) { setState(() => _loading = false); }
  }

  List<_Cancha> get _filtradas => _canchas.where((c) =>
      widget.search.isEmpty ||
      c.nombre.toLowerCase().contains(widget.search.toLowerCase())).toList();
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            const Text('Canchas', style: TextStyle(color: kWhite, fontSize: 18, fontWeight: FontWeight.bold)),
            const Spacer(),
            Text('${_filtradas.length}', style: const TextStyle(color: kLightGray, fontSize: 12)),
            IconButton(icon: const Icon(Icons.refresh_rounded, color: kGreenNeon, size: 20), onPressed: _cargar),
          ]),
          const SizedBox(height: 10),
          if (_loading)
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (ctx, i) {
                  ...
                  final color = c.disponibilidad ? kGreenNeon : Colors.redAccent;
                  return Container(
                    margin: const EdgeInsets.only(bottom: 8),
                      ...
                      Expanded(child: Column(
                        ...
                        children: [
                          Text(c.nombre,
                              style: const TextStyle(color: kWhite, fontWeight: FontWeight.bold, fontSize: 13),
                              overflow: TextOverflow.ellipsis),
                          Text('${c.latitud.toStringAsFixed(4)}, ${c.longitud.toStringAsFixed(4)}',
                              style: const TextStyle(color: kLightGray, fontSize: 10)),
                        ],
                      )),
                      const SizedBox(width: 8),
                      GestureDetector(
                        onTap: () async { await widget.service.toggleDisponibilidad(c.id); _cargar(); },
                        child: Container(
                          ...
                          child: Row(mainAxisSize: MainAxisSize.min, children: [
                            Icon(c.disponibilidad ? Icons.check_circle_rounded : Icons.cancel_rounded,
                                color: color, size: 12),
                            const SizedBox(width: 4),
                            Text(c.disponibilidad ? 'Activa' : 'Inactiva',
...
class _ReservasView extends StatefulWidget {
  ...
  final NumberFormat currency;
  final VoidCallback onRefresh;
  const _ReservasView({required this.service, required this.search,
...
class _ReservasViewState extends State<_ReservasView> {
  ...
  Future<void> _cargar() async {
    try {
      ...
      setState(() { _reservas = data; _loading = false; });
    } catch (_) { setState(() => _loading = false); }
  }

  List<_Reserva> get _filtradas => _reservas.where((r) {
    final ms = widget.search.isEmpty ||
    ...
    final me = _filtroEstado == 'TODOS' || r.estado.toUpperCase() == _filtroEstado;
    return ms && me;
  }).toList();
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            const Text('Reservas', style: TextStyle(color: kWhite, fontSize: 18, fontWeight: FontWeight.bold)),
            const Spacer(),
            Text('${_filtradas.length}', style: const TextStyle(color: kLightGray, fontSize: 12)),
            IconButton(icon: const Icon(Icons.refresh_rounded, color: kGreenNeon, size: 20), onPressed: _cargar),
          ]),
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: ['TODOS', 'RESERVADO', 'PENDIENTE_PAGO', 'CANCELADA', 'FINALIZADA']
                  .map((estado) {
                ...
                return Padding(
                  ...
                  child: GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      child: Text(
                        estado == 'TODOS' ? 'Todos'
                            : estado[0] + estado.substring(1).toLowerCase().replaceAll('_', ' '),
                        style: TextStyle(
                            color: sel ? color : kLightGray, fontSize: 11,
                            fontWeight: sel ? FontWeight.bold : FontWeight.normal),
                      ),
                    ),
                  ),
                  ...
                  return Container(
                    ...
                    child: Row(children: [
                      Container(
                        ...
                        child: Center(child: Text(r.id.toString(),
                      ...
                      Expanded(child: Column(
                        ...
                        children: [
                          Text(r.usuarioNombre,
                              style: const TextStyle(color: kWhite, fontWeight: FontWeight.bold, fontSize: 13),
                              overflow: TextOverflow.ellipsis),
                          Text('${r.canchaNombre} · ${r.fechaReserva} ${r.horaInicio}',
                              style: const TextStyle(color: kLightGray, fontSize: 11),
                              overflow: TextOverflow.ellipsis),
                        ],
                      )),
                      const SizedBox(width: 6),
                      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                        Text(widget.currency.format(r.totalPago),
                            style: const TextStyle(color: kWhite, fontWeight: FontWeight.bold, fontSize: 12)),
                        const SizedBox(height: 4),
                        Container(
                          ...
                          decoration: BoxDecoration(
                              color: color.withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
                          child: Text(
                            r.estado[0] + r.estado.substring(1).toLowerCase().replaceAll('_', ' '),
      ...
      child: Column(
        ...
        children: [
          ...
          ...[
            ...
            ('Roles', 'APP_ADMIN · CANCHA_ADMIN · CLIENTE', Icons.people_rounded),
            ...
            return Container(
              ...
              child: Row(children: [
                Container(
                  width: 36, height: 36,
                  decoration: BoxDecoration(
                      color: kGreenNeon.withOpacity(0.08), borderRadius: BorderRadius.circular(10)),
                ...
                Expanded(child: Column(
                  ...
                  children: [
                    ...
                    Text(value, style: const TextStyle(color: kWhite, fontSize: 13, fontWeight: FontWeight.w500)),
                  ],
                )),
              ]),
        ...
        children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(value,
                ...
                overflow: TextOverflow.ellipsis),
            Text(label, style: const TextStyle(color: kLightGray, fontSize: 10)),
          ]),
        ],
      ),
    ).animate().fadeIn(delay: (index * 60).ms, duration: 400.ms).slideY(begin: 0.15);
  }
}
...
class _ReservaRow extends StatelessWidget {
  ...
  final NumberFormat currency;
  final int index;
  const _ReservaRow({required this.reserva, required this.currency, required this.index});
        ...
        Expanded(child: Column(
          ...
          children: [
            Text(reserva.usuarioNombre,
                style: const TextStyle(color: kWhite, fontWeight: FontWeight.w600, fontSize: 13),
                overflow: TextOverflow.ellipsis),
